...
#define btnUp  2       // Boton para subir en el menu
#define btnDown  4      // Boton para bajar en el menu
...
void loop() {
  ...
  if((btnDownState==HIGH)&&(posicion_encoder>0)) {
    posicion_encoder--;
    Bandera_menu=1;
  }
  btnUpState=digitalRead(btnUp);
  if((btnUpState==HIGH)&&(posicion_encoder<100)){
    Bandera_menu=1;
   posicion_encoder++;
  }
  Actualizacion_menu();
...
void encoder_boton() {
  ...
  // Serial.println(" INTERRUPCION ENCODER");
    ...
    posicion_encoder = 0;
    if(Estado_menu==6) posicion_encoder = Contador_Acumulado;
    Bandera_menu = 1;
